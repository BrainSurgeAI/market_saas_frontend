...
interface ProductImageUploadProps {
  ...
  height?: number;
  disabled?: boolean;
  productCode: string; // 产品编码，用于文件命名
}

/**
 * 产品图片上传组件，支持上传到华为云OBS
 * 仅支持 .webp 格式，并将文件名改为 productCode.webp
 */
export function ProductImageUpload({
  ...
  const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type !== 'image/webp') {
      setError('只能上传 .webp 格式的图片');
      toast({
        title: '文件类型错误',
        description: '只能上传 .webp 格式的图片',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      // 使用 FormData 通过服务端上传，避免跨域问题
      const formData = new FormData();
      formData.append('file', file);
      formData.append('productCode', productCode);
      
      console.log('开始上传文件:', productCode);
      ...
      const { objectUrl } = await uploadResponse.json();
      
      // 上传成功，更新值
      onChange(objectUrl);
      toast({
        title: '上传成功',
        description: '产品图片已成功上传',
      });
