...
import { format, parse, startOfMonth, endOfMonth, eachDayOfInterval, isSameDay, isBefore, getDay, addDays } from "date-fns"
...
export function FutureDatePicker({
  ...
  const parseDate = (dateStr: string) => {
    ...
    // 如果选择的日期在今天之前，则默认为今天
    return isBefore(parsedDate, today) && !isSameDay(parsedDate, today) 
      ? today 
      : parsedDate;
  };
                        ...
                        <div key={`day-${weekIndex}-${dayIndex}`} className="flex-1 flex justify-center">
                          <button
                            ...
                            className={`
                              ...
                              ${isPastDate ? 'text-gray-300 cursor-not-allowed hover:bg-white' : ''}
                            `}
                          >
                            {day.getDate()}
                          </button>
